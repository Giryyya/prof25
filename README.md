...
Если там апаче, то его невозможно отключить, он просто меняет PID и работает дальше, в этом случае можно перенаправить контейнер с 8080 порта на 80:
```
docker run -d --name web -p 8080:80 --restart=always localhost:5000/web:1.0
```

Перед этим необходимо проверить занят ли порт и освободить его:
```
kill PID
```
